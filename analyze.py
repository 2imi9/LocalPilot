"""
Analyze experiment results and generate paper figures.
Compares baseline vs research-enhanced vs quick-search autoresearch runs.

Usage: python analyze.py [results_dir]
  Expects: results_baseline.tsv, results_enhanced.tsv, results_quicksearch.tsv
  Outputs: figures/ directory with paper-ready plots
"""
...
matplotlib.rcParams.update({
    ...
    "figure.dpi": 150,
    "savefig.dpi": 300,
    # "savefig.bbox_inches": "tight",  # not valid in this matplotlib version
})
...
def fig3_time_vs_bpb(data: dict):
    ...
    for label, df in data.items():
        if df is None:
            continue
        # Approximate wall time: 5 min per experiment + 2 min research every 5 (for enhanced)
        time_per_exp = 5  # minutes
...
def table1_summary(data: dict):
    ...
    for label, df in data.items():
        ...
        rows.append({
            ...
            "Avg Memory (GB)": f"{valid['memory_gb'].mean():.1f}" if "memory_gb" in valid.columns and len(valid) > 0 else "N/A",
        })
...
def main():
    ...
    if not available:
        ...
        print("Expected: results_baseline.tsv, results_enhanced.tsv, results_quicksearch.tsv")
        sys.exit(1)
