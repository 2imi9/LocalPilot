...
Override any setting via environment variables or localpilot.yaml.

Usage:
    from config import LocalPilotConfig
    cfg = LocalPilotConfig()
    cfg.print_summary()

    # Or override via yaml:
    # localpilot.yaml:
    #   web_agent: allenai/MolmoWeb-4B
    #   code_agent_gguf: unsloth/Devstral-Small-2-24B-Instruct-2512-GGUF
    #   code_agent_quant: Q4_K_S
"""
...
WEB_AGENT_MODELS = {
    "MolmoWeb-4B": {
        ...
        "vram_gb": 8,
        "description": "4B visual web agent — fits most GPUs",
        "min_vram_gb": 8,
    },
}
...
def recommend_web_agent(vram_gb: float) -> str:
    """Return recommended web agent model key."""
    return "MolmoWeb-4B" if vram_gb >= 8 else None
...
def recommend_code_agent(vram_gb: float) -> str:
    ...
    # After training finishes, full VRAM is available for code agent
...
class LocalPilotConfig:
    ...
    llama_server: Path = field(default_factory=lambda: Path(__file__).parent.parent / "llama.cpp" / "llama-server.exe")
    ...
    def __post_init__(self):
        # Load yaml overrides if present
        cfg_file = Path(__file__).parent / "localpilot.yaml"
        if cfg_file.exists():
