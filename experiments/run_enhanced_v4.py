...
Three-agent pipeline, all local, no cloud APIs:
  1. Qwen3.5-9B plans search query based on experiment history
  2. Research session: Scholar API (fast) + MolmoWeb visual browse (fallback)
  3. Relevance scoring filters papers (Qwen rates 0-10)
  4. Qwen3.5-9B proposes PARAM + exact VALUE citing research
  5. Value clamped to safe bounds, OOM checked
  6. Standard greedy keep/discard training loop
...
# Training runs in Docker (Linux + CUDA + FA3 kernel)
DOCKER_IMAGE = "autoresearch-train"
# Convert Windows path to Docker-compatible mount path
_root_posix = str(ROOT).replace("\\", "/")
# For Docker Desktop on Windows, C:\Users\... -> /c/Users/... or //c/Users/...
if sys.platform == "win32" and len(_root_posix) > 1 and _root_posix[1] == ":":
    _docker_mount = "/" + _root_posix[0].lower() + _root_posix[2:]
else:
    _docker_mount = _root_posix
...
def score_relevance(finding, max_tokens=10):
    """Score how relevant a research finding is to our task. Returns float [0, 1]."""
...
Output ONLY a single number (0-10):"""
    ...
    response = call_llm(prompt, max_tokens=10, temperature=0.0)
...
def score_and_filter_papers(papers):
    ...
    for p in papers:
        ...
        print(f"    {score:.2f} | {title_short}")
        time.sleep(0.5)  # small delay between scoring calls
    ...
    try:
        while True:
            ...
            if len(queue) < REFILL_THRESHOLD:
                ...
                # Qwen scores each paper, partitions into skip/summary/deep-read
                start_llama_server(QWEN_MODEL, ctx_size=4096, label="Qwen3.5-9B scorer")
                ...
                for paper_info in deep_urls[:MOLMOWEB_MAX_PAPERS]:
                    ...
                    time.sleep(API_RATE_DELAY)  # breathing room between pages
